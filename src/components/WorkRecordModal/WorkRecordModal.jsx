...
export function WorkRecordModal({
  ...
  function handleSave() {
    if (!validate()) return;
    onSave({ hours: numHours, minutes: numMinutes, hourlyRate: numRate, taxRate: numTax, note: note.trim() });
  }

  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/40" onClick={onClose}>
      <div
        className="w-full sm:max-w-md bg-[var(--paper-raised)] rounded-t-3xl sm:rounded-3xl px-5 pt-5 pb-6 max-h-[92vh] overflow-y-auto"
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <h2 className="text-[18px] font-semibold text-[var(--ink)]">{dateLabel}</h2>
          </div>
